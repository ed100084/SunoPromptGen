...
import { createQuickRandomDraft, type RandomnessLevel } from './quickRandom';
  ...
  return <div className="min-h-screen bg-[#f6f5fb] text-slate-900 dark:bg-[#090b13] dark:text-slate-100">
    ...
    <main className="mx-auto max-w-[1500px] px-4 py-6 lg:px-8">
      <WorkspaceStageNav
        ...
        variant="collapsible"
      />
      <div className="mb-5 rounded-3xl bg-gradient-to-r from-violet-600 to-fuchsia-600 p-5 text-white shadow-lg">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-violet-100">Quick create</p><h2 className="mt-1 text-xl font-bold">不用填完所有欄位，先產生一個完整方向</h2><p className="mt-1 text-sm text-violet-100">保留歌名與歌詞，其餘由協調過的組合快速填入。</p></div><button onClick={() => { dispatch({ type: 'replace', draft: createQuickRandomDraft(draft, randomness) }); setSelectedPresetId(null); setStatusMessage(`已產生${randomness === 'safe' ? '穩定' : randomness === 'wild' ? '大膽' : '平衡'}隨機方向。`); }} className="shrink-0 rounded-2xl bg-white px-6 py-3 text-base font-black text-violet-700 shadow hover:bg-violet-50">🎲 快速隨機</button></div>
        <div className="mt-4 flex flex-wrap items-center gap-2"><span className="text-xs font-semibold">隨機程度</span>{(['safe', 'balanced', 'wild'] as const).map((level) => <button key={level} onClick={() => setRandomness(level)} className={`rounded-full px-3 py-1.5 text-xs font-bold ${randomness === level ? 'bg-white text-violet-700' : 'bg-white/15 text-white'}`}>{level === 'safe' ? '穩定' : level === 'balanced' ? '平衡' : '大膽'}</button>)}<button onClick={() => setAdvancedOpen((open) => !open)} className="ml-auto rounded-full border border-white/40 px-3 py-1.5 text-xs font-semibold">{advancedOpen ? '收起進階設定' : '展開進階設定'}</button></div>
      </div>
      {advancedOpen && <WorkflowPicker value={draft.workflow} onChange={(value) => set('workflow', value)} />}
      {advancedOpen && <PresetPicker value={selectedPresetId} onSelect={applyPresetToDraft} className="mb-5" />}
      ...
      <div className="grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)_minmax(360px,0.85fr)]">
        ...
        <section className="space-y-5">
          {advancedOpen && <ModelStrategyPicker value={draft.model} onChange={(value) => set('model', value)} />}

          <div id="stage-brief" className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><p className="text-xs font-semibold uppercase tracking-wider text-violet-600">02 · Creative brief</p><h2 className="mb-5 mt-1 text-xl font-bold">先說清楚要創作什麼</h2><div className="space-y-4"><Field label="作品名稱"><input className={inputClass} value={draft.title} onChange={(e) => set('title', e.target.value)} placeholder="未命名作品" /></Field><Field label="一句話創作意圖" hint="主題、情境與情緒轉變，比曲風標籤更重要"><textarea className={inputClass} rows={3} value={draft.concept} onChange={(e) => set('concept', e.target.value)} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="曲風"><input className={inputClass} value={draft.genres} onChange={(e) => set('genres', e.target.value)} placeholder="例如 cinematic pop" /></Field><Field label="情緒"><input className={inputClass} value={draft.moods} onChange={(e) => set('moods', e.target.value)} placeholder="例如溫暖、懷舊" /></Field></div>{advancedOpen && <><div className="grid gap-4 sm:grid-cols-2"><Field label="語言"><input className={inputClass} value={draft.language} onChange={(e) => set('language', e.target.value)} /></Field><Field label="Tempo"><input className={inputClass} value={draft.tempo} onChange={(e) => set('tempo', e.target.value)} /></Field></div><Field label="Overall feel / 場景"><textarea className={inputClass} rows={3} value={draft.overallFeel} onChange={(e) => set('overallFeel', e.target.value)} /></Field></>}</div></div>
          ...
          {advancedOpen && draft.workflow === 'explore' && <div className="rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-5 dark:border-fuchsia-900 dark:bg-fuchsia-950/20"><h2 className="mb-3 font-bold">探索軸線</h2><textarea className={inputClass} rows={3} value={draft.exploration} onChange={(e) => set('exploration', e.target.value)} /></div>}
          ...
          {advancedOpen && <Suspense fallback={<div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900">載入 AI 歌詞工具…</div>}><LyricsAiPanel
            lyrics={draft.lyrics}
