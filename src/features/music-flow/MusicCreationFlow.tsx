...
import {
  buildExternalLyricsPrompt, buildSunoOutput, createArrangement, parseAndReviewLyrics,
  suggestStyleFromReference, type FlowSection, type ReferenceInput, type StyleFacets,
} from './musicFlow';

const input = 'w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-violet-500 dark:border-slate-700 dark:bg-slate-950';
...
export function MusicCreationFlow() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><h2 className="text-lg font-bold">2 · 調整風格</h2><p className="mt-1 text-xs text-amber-600">產品轉譯建議，不是 Suno 官方規格。中文意圖會整理成較適合 Style 欄位的音樂特徵。</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{([['genre','曲風／核心參考'],['mood','情緒'],['instruments','樂器與角色'],['vocal','人聲'],['tempo','速度／律動'],['production','製作與音色'],['avoid','避免事項']] as Array<[keyof StyleFacets,string]>).map(([key,label]) => <label key={key} className={key === 'genre' || key === 'production' ? 'sm:col-span-2' : ''}><span className="mb-1 block text-xs font-bold">{label}</span><textarea rows={key === 'genre' ? 2 : 1} className={input} value={style[key]} onChange={(e) => updateStyle(key, e.target.value)} /></label>)}</div></section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><h2 className="text-lg font-bold">4 · 交給其他 AI 填詞，再貼回來</h2><div className="mt-4 grid gap-4 lg:grid-cols-2"><div><div className="mb-2 grid grid-cols-2 gap-2"><input className={input} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="歌名" /><input className={input} value={language} onChange={(e) => setLanguage(e.target.value)} placeholder="語言" /></div><textarea className={input} rows={3} value={concept} onChange={(e) => setConcept(e.target.value)} placeholder="歌詞主題／故事" /><textarea readOnly className={`${input} mt-2 font-mono text-xs`} rows={14} value={lyricsPrompt} /><div className="mt-2"><CopyButton value={lyricsPrompt} label="複製填詞 Prompt" /></div></div><div><textarea className={input} rows={18} value={lyrics} onChange={(e) => setLyrics(e.target.value)} placeholder="把其他 AI 生成的完整歌詞貼在這裡…" />{lyrics.trim() && <div className={`mt-2 rounded-xl p-3 text-xs ${review.issues.length ? 'bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:text-amber-200' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-200'}`}><strong>{review.issues.length ? `發現 ${review.issues.length} 項待確認` : '✓ 段落結構完整'}</strong>{review.issues.length > 0 && <ul className="mt-1 list-disc pl-4">{review.issues.map((issue) => <li key={issue}>{issue}</li>)}</ul>}</div>}</div></div></section>

      <section className="rounded-3xl border-2 border-violet-300 bg-white p-5 dark:border-violet-800 dark:bg-slate-900"><h2 className="text-xl font-black">5 · 最終檢視與 Suno 輸出</h2><div className="mt-4 flex flex-wrap gap-4 text-sm"><label><input type="checkbox" checked={instrumental} onChange={(e) => setInstrumental(e.target.checked)} /> 純音樂</label><label>模型 <select className="ml-2 rounded border bg-transparent px-2 py-1" value={model} onChange={(e) => setModel(e.target.value)}><option>v6</option><option>v6-wild</option><option>v6-mini</option></select></label></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><div><div className="mb-2 flex items-center justify-between"><strong>Style</strong><CopyButton value={output.style} label="複製 Style" /></div><textarea readOnly rows={9} className={`${input} font-mono text-xs`} value={output.style} /><p className="mt-2 text-xs text-slate-500">Exclude styles：{output.settings.excludeStyles || '無'}</p></div><div><div className="mb-2 flex items-center justify-between"><strong>Custom Lyrics</strong><CopyButton value={output.customLyrics} label="複製 Custom Lyrics" /></div><textarea readOnly rows={14} className={`${input} font-mono text-xs`} value={output.customLyrics} /></div></div><div className="mt-4 rounded-xl bg-slate-950 p-4 text-xs text-slate-200"><strong>Custom 設定</strong><pre className="mt-2 whitespace-pre-wrap">{JSON.stringify(output.settings, null, 2)}</pre></div></section>
    </main>
  </div>;
}
