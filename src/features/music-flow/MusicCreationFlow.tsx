...
export function MusicCreationFlow() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><h2 className="text-lg font-bold">2 · 調整風格</h2><p className="mt-1 text-xs text-amber-600">產品轉譯建議，不是 Suno 官方規格。中文意圖會整理成較適合 Style 欄位的音樂特徵。</p><div className="mt-4"><p className="mb-2 text-xs font-bold">先選一個情緒樣板，再直接修改文字</p><div className="flex flex-wrap gap-2">{MOOD_TEMPLATES.map((template) => <button key={template.id} type="button" title={template.description} onClick={() => updateStyle('mood', template.value)} className={`rounded-full border px-3 py-2 text-xs font-semibold transition ${style.mood === template.value ? 'border-violet-500 bg-violet-600 text-white' : 'border-slate-200 bg-slate-50 hover:border-violet-300 dark:border-slate-700 dark:bg-slate-800'}`}>{template.name}</button>)}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{([['genre','曲風／核心參考'],['mood','情緒（可自由修改）'],['instruments','樂器與角色'],['vocal','人聲'],['tempo','速度／律動'],['production','製作與音色'],['avoid','避免事項']] as Array<[keyof StyleFacets,string]>).map(([key,label]) => <label key={key} className={key === 'genre' || key === 'production' ? 'sm:col-span-2' : ''}><span className="mb-1 block text-xs font-bold">{label}</span><AutoTextarea minRows={key === 'genre' || key === 'mood' || key === 'production' || key === 'avoid' ? 3 : 2} value={style[key]} onChange={(e) => updateStyle(key, e.target.value)} /></label>)}</div></section>
